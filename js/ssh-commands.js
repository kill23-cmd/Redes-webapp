...
`
    ...
    /**
     * Show proactive insight generated by background AI
     */
    showBackgroundInsight(win, insightText) {
        ...
        if (container) {
            ...
            // Clear if "aguardando" is there
            const chatMessages = win.document.getElementById('chat-messages');
            if (chatMessages && chatMessages.innerHTML.includes('aguarde')) {
                chatMessages.innerHTML = '';
            }
            
            this.addChatMessage(win, 'assistant', "🚨 **Insight Automático Pós-Comando:**\n\n" + insightText);
            
            // Add to history so chat flow makes sense
            if (!win.chatHistory) win.chatHistory = [];
            win.chatHistory.push({ role: 'assistant', content: insightText });
        }
    }
